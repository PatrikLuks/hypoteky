...
const PripadCard: React.FC<PripadCardProps> = React.memo(({ pripad, onEdit, onDelete, onArchive, onUnarchive, onShowWorkflow }) => {
  ...
  const [swiped, setSwiped] = React.useState<'left'|'right'|null>(null);
  ...
  return (
    <motion.div {...(isTouchDevice ? swipeHandlers : {})} animate={controls}>
        ...
        <CardContent>
          ...
          {isTouchDevice && (
            <Box sx={{mt:1, fontSize:'0.95em', color:'#888', textAlign:'center'}}>
              <span style={{marginRight:8}}>← swipe pro archivaci</span>
              <span>swipe → pro {pripad.archivovano ? 'obnovení' : 'smazání'}</span>
            </Box>
          )}
        </CardContent>
      </Card>
